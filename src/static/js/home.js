...
function handleHomeProgressStatus(status) {
  ...
  if (status == MINDGAME_PROGRESS_ENUM.CPT) {
    SetText(g_restore_btn, "Restore: CPT");
    SetDst(g_restore_btn, "/cpt");
  }
...
// handles the clicking of INLAB button
g_inlab_btn.addEventListener("click", async () => {
  ...
  /////////////////////////// AIDAN ADD ## stores inlab selection /////////////////////////
  localStorage.setItem(LOCALSTORAGE_LOCATION, "INLAB");
...
// Handle the clicking of ATHOME button
g_athome_btn.addEventListener("click", async () => {
  ...
  localStorage.setItem(LOCALSTORAGE_USERID, userID);
  localStorage.setItem(LOCALSTORAGE_LOCATION, "HOME");
